    ...
    function(d3, d3tip) {
        ...
        var timeseries = function() {
            ...
            var chart = function(elem) {
                ...
                var xAxis = d3.axisBottom(xscale).tickFormat(xscale.setformat)
                var yAxis = d3.axisLeft(yscale).tickFormat(yscale.setformat)

                /*brush.x(fullxscale).on('brush', function() {
                ...
                //interaction with the timeline; allows the user to select date ranges to display data from
                var brush = d3.brushX()
                    .extent([0,0],[width, drawerHeight])
                    .on("brush", function() {
                        if(!d3.event.sourceEvent) return;
                        var s = d3.event.selection || fullxscale.range();
                        xscale.domain(s.map(fullxscale.invert, fullxscale));
                        drawerContainer.select(".area").attr("d", area);
                        //xscale.domain(!d3.event.sourceEvent ? fullxscale.domain() : brush.extent());
                        series.forEach(drawSerie)
                        svg.select(".focus.x.axis").call(xAxis);
                        mousevline.update()
                        updatefocusRing()
